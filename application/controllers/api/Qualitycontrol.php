...
class qualitycontrol extends REST_Controller {
    ...
    public function index_get()
    {
        ...
        $id = $this->get('ID');
        if($id === null){
            $data = $this->costeng->getCosteng('data_qc');
        }else{

            $data = $this->costeng->getCosteng('data_qc',$id,'ID');
        }
    ...
    public function index_delete(){
        ...
        if( $id === null){
            ...
            if( $this->costeng->deleteCosteng('data_qc',$id,'ID') > 0){
                $this->set_response([
                    'status' => true,
                    'data' => $id,
                    'message' => 'deleted'
    ...
    public function index_post(){
        $isi = [
            ...
            'Nama_mesin' => $this->post('Nama_mesin'),
            'Time' => date('H:i:s'),
            'PJ'=> $this->post('PJ'),
            'Quality' => $this->post('Quality'),
            'Performance' => $this->post('Performance'),
            'Availibility' => $this->post('Availibility'),
        ...
        if ($this->costeng->createCosteng('data_qc',$isi)> 0){
            $this->set_response([
                'status' => true,
    ...
    public function index_put(){
        ...
        $isi = [
            ...
            'Performance' => $this->put('Performance'),
            'Availibility' => $this->put('Availibility'),
